...
const SelectSeatsStep = ({navigation, route}) => {
  ...
  const initialMapping = () => {
    ...
    for (let i = 0; i < show.hallHeight; i++) {
      for (let j = 0; j < show.hallWidth; j++) {
  ...
  React.useEffect(() => {
    ...
    const selectedSeats = temp.filter(
      item => item.status === Status.Selected,
    ).length;

    if (selectedSeats < seatsQty) {
      const item = temp.find(seat => seat.seatId === selectedSeat.seatId);
      const index = temp.indexOf(item);
      if (~index) {
        temp[index] = selectedSeat;
      }
      setSeatMap(temp);
    }
  }, [selectedSeat]);
